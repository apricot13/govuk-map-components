...
export class MapComponent extends HTMLElement {
  ...
  constructor() {
    ...
    this._controllers.push(
      new EditModeController(this, {
        showFeature: this.#showFeature.bind(this),
      })
    );
  ...
  connectedCallback(): void {
    ...
    this.reactiveStore.editMode.subscribe(
      (editMode: ModeName | "magic_wand") => {
    ...
    this.reactiveStore.tileType.subscribe((tileType: "os" | "satellite") => {
      this.classList.toggle("map--satellite-tiles", tileType === "satellite");
    });
    ...
    this.baseMap?.addEventListener("geomanLoaded", () => {
    ...
    this.baseMap?.addEventListener("geomanEvent", (event: Event) => {
      const customEvent = event as CustomEvent<{
        featureStore: any;
        parameters: { name: string; isManualDelete?: boolean };
      }>;
      const allowedEvents = [
        "gm:dragend",
        "gm:editend",
      ...
      if (allowedEvents.includes(customEvent.detail.parameters.name)) {
        ...
        const isManualDelete =
          customEvent.detail.parameters.isManualDelete === true;
        if (isManualDelete) {
    ...
    this.innerHTML = `
      ...
      <div class="map__main-container">
        <div class="map__map-libre">
          ...
          <base-map ${this.hasAttribute("data-map-styles-url") ? `data-map-styles-url="${this.getAttribute("data-map-styles-url")}"` : ""} ${this.hasAttribute("data-map-styles-satellite-url") ? `data-map-styles-satellite-url="${this.getAttribute("data-map-styles-satellite-url")}"` : ""} data-edit-enabled="${this.hasAttribute("data-save-endpoint") ? "true" : "false"}"></base-map>
        </div>
        ${
