...
export class EditModeController {
  ...
  constructor(host: MapComponent, capabilities: Record<string, any>) {
    this.host = host;
  ...
  #render() {
    const buttonsContainer = this.host.querySelector(
      ".map__buttons-left"
  ...
  #addEventListeners() {
    const editButton = this.host.querySelector(
      ".map__edit-button"
    ) as HTMLButtonElement | null;
    const cancelButton = this.host.querySelector(
      ".map__cancel-button"
    ) as HTMLButtonElement | null;
    const resetButton = this.host.querySelector(
    ...
    editButton?.addEventListener("click", () => {
      ...
      const changeRadio = this.host.querySelector(
        ".map__edit-map-change-input"
      ) as HTMLInputElement | null;
      const dragRadio = this.host.querySelector(
        ".map__edit-map-drag-input"
      ) as HTMLInputElement | null;
    ...
    this.host
      .querySelector(".map__cancel-reset-confirm-button")
    ...
    this.host
      .querySelector(".map__confirm-reset-button")
      ?.addEventListener("click", () => {
        this.#confirmReset();
      });
    ...
    this.host
      .querySelector(".map__confirm-cancel-button")
      ?.addEventListener("click", () => {
        this.#closeCancelConfirm();
        this.#performCancel(editButton);
      });
    this.host
      .querySelector(".map__cancel-cancel-confirm-button")
      ?.addEventListener("click", () => {
        this.#closeCancelConfirm();
        window.setTimeout(() => {
          cancelButton?.focus();
    ...
    this.host
      .querySelector(".map__save-button")
      ?.addEventListener("click", async () => {
        ...
        if (!ok) {
          ...
          (
            this.host.querySelector(".map__save-error") as HTMLElement | null
          )?.focus();
          return;
        }
    ...
    this.host
      .querySelector(".map__undo-button")
      ?.addEventListener("click", () => {
        this.#handleHistoryAction("undo");
      });

    this.host
      .querySelector(".map__redo-button")
      ?.addEventListener("click", () => {
  ...
  #confirmReset() {
    ...
    try {
      try {
        ...
        this.host.historyManager?.addState(
          this.host.baseMap?.collectFeatureEntries()
        );
        this.host.historyManager?.clear();
        this.host.classList.remove("map--has-undos", "map--has-redos");
      } finally {
  ...
  #performCancel(editButton: HTMLButtonElement | null) {
    ...
    this.host
      .querySelector<HTMLButtonElement>(".map__reset-shape-button")
  ...
  async #saveFeatures(): Promise<boolean> {
    ...
    if (allFeatures.length === 0) {
      this.#showSaveError(
        "There are no shapes to save. Add a shape or cancel."
      );
      return false;
    }
    ...
    const expectedBaseType = originalType.includes("Polygon")
      ? "Polygon"
  ...
  #showSaveError(message: string) {
    this.host.classList.add("map--save-error");
    const body = this.host.querySelector(
      `#save-error-body-${this.host._id}`
